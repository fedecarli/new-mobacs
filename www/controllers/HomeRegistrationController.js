angular.module('mobacs').controller('HomeRegistrationController', function($scope, $ionicPlatform, $location, GlobalService, $state, HomeRegistrationService, $rootScope, $stateParams, $ionicHistory, $ionicPopup, FamilyService, $cordovaGeolocation, completaLogradouroService) {
    ...
    $scope.data = { "tiposDeLogradouros": [], "pesquisaTipos": false };

    $scope.completaLogradouro() = function() {

        completaLogradouroService.searchTiposDeLogradouros($scope.dataHomeRegistration.tipoLogradouroNumeroDne).then(
            function(matches) {
                if (matches.length < 5) {
                    $scope.data.tiposDeLogradouros = matches;
                    $scope.data.pesquisaTipos = true;
                } else {
                    $scope.data.pesquisaTipos = false;
    ...
    $scope.selectTipoDeLog = function(listaTipoDeLog) {
        $scope.dataHomeRegistration.tipoLogradouroNumeroDne = listaTipoDeLog.descricao;
        $scope.data.pesquisaTipos = false;
    ...
    $scope.searchAdressInformation = function(string) {
        ...
        if (typeof string !== 'undefined') {
            string = string.replace('-', '');
            if (string == '18055-088') {
                $scope.dataHomeRegistration.codigoIbgeMunicipio = 'ITANHAÉM';
                $scope.dataHomeRegistration.numeroDneUf = '26';
            } else {
                $scope.dataHomeRegistration.codigoIbgeMunicipio = null;
                $scope.dataHomeRegistration.numeroDneUf = null;
                ...
                $ionicPopup.alert({
                    title: 'CEP Inválido',
                    template: 'Apenas são aceitos CEPs válidos de Itanhaém'
                });
            }
        }
    };
