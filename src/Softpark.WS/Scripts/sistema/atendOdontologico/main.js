...
function cadAtendOdontologico() {
    $("#btnCadAtendimentoOdontologico").prop('disabled', true);
    jQuery.ajax({
        type: "POST",
        data: {
            acao: "create",
    ...
    .done(function (data) {
        ...
        if (id > 0) {
            window.location.assign("atendimentoOdontologicoCad.asp");
        }
        else {
...
function editAtendOdontologico(id) {
    jQuery.ajax({
        type: "POST",
        data: {
            acao: "transf", id: id },
...
$(document).ready(function () {
    $('#listaAtendOdontologico').dataTable({
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": "ajax/dataTables/?xQ=listaAtendOdontologico",
            "type": "POST"
        },
        "language": {
        ...
        "fnDrawCallback": function () {
            ...
            if (permCad) $('.dataTables_filter').prepend('<button type="button" id="btnCadAtendimentoOdontologico" class="btn btn-success pull-right btn-datatable" onclick="cadAtendOdontologico();">Novo Cadastro</button>');

            if (permAtu) {
                $("#btnCadAtendimentoOdontologico").prop('disabled', false);
                $("#listaAtendOdontologico .btn-outline").click(function () {
                    $("#atendOdontologico").val($(this).attr("data-id"));
                    $("#listaAtendOdontologico").submit();
                    editAtendOdontologico($(this).data("id"));
                });
            } else {
                $('#listaAtendOdontologico .permAtu center').hide();
            }
        }
