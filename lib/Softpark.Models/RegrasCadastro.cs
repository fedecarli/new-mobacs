...
namespace Softpark.Models
{
    ...
    public static class RegrasCadastro
    {
        ...
        public static async Task Validar(this UnicaLotacaoTransport header, DomainContainer c)
        {
            if (!header.profissionalCNS.isValidCns())
                throw new ValidationException("CNS inválido.");

            var profissional = c.VW_Profissional.Where(x => x.CNS != null && x.CNS.Trim() == header.profissionalCNS.Trim()).ToArray();
            ...
            if (await c.Cidade.AllAsync(x => x.CodIbge == null || x.CodIbge.Trim() != header.codigoIbgeMunicipio.Trim()))
        ...
        /// <exception cref="ValidationException"></exception>
        public static async Task Validar(this CondicoesDeSaude cond, CadastroIndividual cad, DomainContainer c)
        {
            ...
            if (cond.situacaoPeso != null && !await c.TP_Consideracao_Peso.AnyAsync(x => x.codigo == cond.situacaoPeso))
        ...
        /// <param name="cond"></param>
        /// <exception cref="ValidationException"></exception>
        public static async Task Validar(this EmSituacaoDeRua cond, DomainContainer c)
        {
            ...
            var higienes = cond.HigienePessoalSituacaoRua.Select(x => x.codigo_higiene_pessoal);
            var allh = await c.TP_Higiene_Pessoal.Select(x => x.codigo).ToListAsync();

            if (higienes.Any(x => allh.All(y => y != x)))
            ...
            var origens = cond.OrigemAlimentoSituacaoRua.Select(x => x.id_tp_origem_alimento);
            var allo = await c.TP_Origem_Alimentacao.Select(x => x.codigo).ToListAsync();

            if (origens.Any(x => allo.All(y => y != x)))
                throw new ValidationException("Uma ou mais condições de higiene pessoal não foram encontradas.");
            ...
            if (cond.quantidadeAlimentacoesAoDiaSituacaoRua != null && await c.TP_Quantas_Vezes_Alimentacao.AnyAsync(x => x.codigo != cond.quantidadeAlimentacoesAoDiaSituacaoRua))
            ...
            if (cond.tempoSituacaoRua != null && await c.TP_Sit_Rua.AnyAsync(x => x.codigo != cond.tempoSituacaoRua))
        ...
        public static async Task Validar(this IdentificacaoUsuarioCidadao cond, CadastroIndividual cad, DomainContainer c)
        {
            ...
            if (cond.codigoIbgeMunicipioNascimento != null)
            {
                ...
                if (await c.Cidade.AllAsync(x => x.CodIbge != cond.codigoIbgeMunicipioNascimento))
            ...
            if (await c.TP_Nacionalidade.AllAsync(x => x.codigo != cond.nacionalidadeCidadao))
            ...
            if (await c.TP_Raca_Cor.AllAsync(x => x.id_tp_raca_cor != cond.racaCorCidadao))
                throw new ValidationException("A Raça/Cor do cidadão é inválida.");

            if (await c.TP_Sexo.AllAsync(x => x.codigo != cond.sexoCidadao))
                throw new ValidationException("O Sexo do cidadão é inválido.");

            if (cond.racaCorCidadao == 5 && (cond.etnia == null || await c.Etnia.AllAsync(x => x.CodEtnia != cond.etnia)))
        ...
        /// <param name="cad"></param>
        /// <exception cref="ValidationException"></exception>
        public static async Task Validar(this InformacoesSocioDemograficas cond, CadastroIndividual cad, DomainContainer c)
        {
            ...
            var deficiencias = cond.DeficienciasCidadao.Select(x => x.id_tp_deficiencia_cidadao);
            var alld = await c.TP_Deficiencia.Select(x => x.codigo).ToListAsync();

            if (deficiencias.Any(x => alld.All(y => y != x)))
                throw new ValidationException("Uma ou mais deficiências estão incorretas.");

            if (cond.grauInstrucaoCidadao != null && await c.TP_Curso.AllAsync(x => x.codigo != cond.grauInstrucaoCidadao))
                throw new ValidationException("O grau de instrução do cidadão é inválido.");

            var profs = await c.AS_ProfissoesTab.ToListAsync();

            if (cond.ocupacaoCodigoCbo2002 != null && profs.All(x => x.CodProfTab == null || x.CodProfTab.Trim() != cond.ocupacaoCodigoCbo2002.Trim()))
            ...
            if (cond.orientacaoSexualCidadao != null && await c.TP_Orientacao_Sexual.AllAsync(x => x.codigo != cond.orientacaoSexualCidadao))
            ...
            if (cond.relacaoParentescoCidadao != null && await c.TP_Relacao_Parentesco.AllAsync(x => x.codigo != cond.relacaoParentescoCidadao)
                    && !cad.IdentificacaoUsuarioCidadao1.statusEhResponsavel)
                throw new ValidationException("A relação de parentesco é inválida.");

            if (cond.situacaoMercadoTrabalhoCidadao != null && await c.TP_Sit_Mercado.AllAsync(x => x.codigo != cond.situacaoMercadoTrabalhoCidadao))
        ...
        /// <param name="cad"></param>
        /// <exception cref="ValidationException"></exception>
        public static async Task Validar(this CadastroIndividual cad, DomainContainer c)
        {
            ...
            await cad.CondicoesDeSaude1?.Validar(cad, c);
            ...
            await cad.EmSituacaoDeRua1?.Validar(c);
            ...
            await cad.IdentificacaoUsuarioCidadao1?.Validar(cad, c);
            ...
            await cad.InformacoesSocioDemograficas1?.Validar(cad, c);
        ...
        /// <param name="cad"></param>
        /// <exception cref="ValidationException"></exception>
        public static async Task Validar(this CondicaoMoradia cond, CadastroDomiciliar cad, DomainContainer c)
        {
            var proibido = (new long[] { 7, 8, 9, 10, 11 }).Contains(cad.tipoDeImovel);

            if (cond.abastecimentoAgua != null && await c.TP_Abastecimento_Agua.AllAsync(x => x.codigo != cond.abastecimentoAgua))
            ...
            if (cond.destinoLixo != null && await c.TP_Destino_Lixo.AllAsync(x => x.codigo != cond.destinoLixo))
                throw new ValidationException("O destino do lixo é inválido.");

            if (cond.formaEscoamentoBanheiro != null && await c.TP_Escoamento_Esgoto.AllAsync(x => x.codigo != cond.formaEscoamentoBanheiro))
                throw new ValidationException("A forma de escoamento do banheiro ou sanitário é inválida.");

            if (cond.localizacao != null && await c.TP_Localizacao.AllAsync(x => x.codigo != cond.localizacao))
            ...
            if (cond.materialPredominanteParedesExtDomicilio != null && await c.TP_Construcao_Domicilio.AllAsync(x => x.codigo != cond.materialPredominanteParedesExtDomicilio))
            ...
            if (cond.situacaoMoradiaPosseTerra != null)
            {
                ...
                if (await c.TP_Situacao_Moradia.AllAsync(x => x.codigo != cond.situacaoMoradiaPosseTerra))
            ...
            if (cond.tipoAcessoDomicilio != null)
            {
                ...
                if (await c.TP_Acesso_Domicilio.AllAsync(x => x.codigo != cond.tipoAcessoDomicilio))
            ...
            if (cond.tipoDomicilio != null)
            {
                ...
                if (await c.TP_Domicilio.AllAsync(x => x.codigo != cond.tipoDomicilio))
                    throw new ValidationException("O tipo de domicílio é inválido.");
            }

            if (cond.aguaConsumoDomicilio != null && await c.TP_Tratamento_Agua.AllAsync(x => x.codigo != cond.aguaConsumoDomicilio))
        ...
        /// <param name="cad"></param>
        /// <exception cref="ValidationException"></exception>
        public static async Task Validar(this EnderecoLocalPermanencia cond, CadastroDomiciliar cad, DomainContainer c)
        {
            ...
            if (cond.codigoIbgeMunicipio == null || await c.Cidade.AllAsync(x => x.CodIbge != cond.codigoIbgeMunicipio))
            ...
            if (cond.numeroDneUf == null || (await c.UF.OrderBy(x => x.DesUF).ToListAsync()).Select(RowNumberPad('0', 2)).All(x => x != cond.numeroDneUf))
            ...
            if (cond.tipoLogradouroNumeroDne == null || await c.TB_MS_TIPO_LOGRADOURO.AllAsync(x => x.CO_TIPO_LOGRADOURO == null || x.CO_TIPO_LOGRADOURO.Trim() != cond.tipoLogradouroNumeroDne))
        ...
        /// <returns></returns>
        /// <exception cref="ValidationException"></exception>
        public static async Task Validar(this CadastroDomiciliar cad, DomainContainer c)
        {
            if (await c.TP_Imovel.AllAsync(x => x.codigo != cad.tipoDeImovel))
            ...
            await cad.CondicaoMoradia1?.Validar(cad, c);
            ...
            await cad.EnderecoLocalPermanencia1?.Validar(cad, c);
